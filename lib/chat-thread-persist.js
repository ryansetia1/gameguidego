...
export async function fetchNormalizedThread(supabase, chatId) {
  ...
  const [{ data: responses }, { data: states }] = await Promise.all([
    supabase
      .from("chat_responses")
      .select(
        "turn_id, variant_index, content, sources, highlights, spoilers, pipeline_type, illustration",
      )
      .in("turn_id", turnIds),
    supabase
...
export async function resolveThreadMessages(supabase, chat) {
  ...
  const picked = pickRicherThread(normalized, legacy);
  return mergeAssistantFieldsFromLegacy(picked, legacy.length ? legacy : normalized);
}
 ...
 * @param {string} chatId
 */
export async function clearNormalizedThread(supabase, chatId) {
  if (!chatId) return { ok: false, reason: "no_chat" };
  const { error } = await supabase.from("chat_turns").delete().eq("chat_id", chatId);
  if (error) return { ok: false, reason: "delete_failed", error };
  return { ok: true };
}
...
async function upsertPairedTurn(
  ...
  if (!assistant) {
    const { error: clearError } = await supabase
      ...
      .eq("turn_id", turn.id);
    if (clearError) {
      return { ok: false, reason: "response_clear", error: clearError };
    }
    await supabase.from("chat_turn_state").delete().eq("turn_id", turn.id);
    return { ok: true };
  }
  ...
  const { error: responseError } = await supabase.from("chat_responses").upsert(
    variantRows.map((row) => ({
      ...
      illustration: row.body.illustration ?? null,
  ...
  const { error: pruneError } = await supabase
    ...
    .delete()
    .eq("turn_id", turn.id)
    .gte("variant_index", variantRows.length);
  if (pruneError) {
    return { ok: false, reason: "response_prune", error: pruneError };
  }
