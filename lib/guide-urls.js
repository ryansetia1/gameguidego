 ...
 * @returns {string}
 */
export function guideUrlHostLabel(url, metaByUrl = {}) {
  if (isUploadedGuideUrl(url)) return uploadedGuideFilename(url);
  if (isActiveGamefaqsBundle(url, metaByUrl[url])) return "GameFAQs bundle";
  try {
...
/** Answer-source / footnote label for a linked preferred guide URL. */
export function guideSourceLinkLabel(url) {
  if (isGamefaqsBundleUrl(url)) return "GameFAQs bundle";
...
export function guideUrlsSummary(urls, metaByUrl = {}) {
  ...
  if (urls.length === 1) {
    ...
    return guideUrlHostLabel(urls[0], metaByUrl);
  }
  ...
  if (uploads.length && links.length) {
    ...
    const linkPart =
      links.length === 1
        ? guideUrlHostLabel(links[0], metaByUrl)
        : `${links.length} links`;
