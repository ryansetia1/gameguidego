  ...
  for (const row of results) {
    ...
    if (pageCount > 0 && pagesIndexed < pageCount) {
      ...
      if (missingPages.length) {
        ...
        const blocked = missingPages.filter((page) => page.reason === "blocked").length;
        const reasonNote =
          blocked > 0 && blocked === missingPages.length ? " (blocked by GameFAQs)" : "";
        return `Indexed ${pagesIndexed} of ${pageCount} bundle pages. Couldn't add: ${names}${more}${reasonNote}.`;
      }
