 ...
 * @param {Array<{ url: string }> | undefined} sources
 */
export function linkedSourceHosts(sources) {
  ...
  for (const source of sources ?? []) {
    ...
    const label = guideSourceLinkLabel(source.url);
...
export function pipelineSourceLabel(pipelineType, sources) {
  ...
  if (uploadLabel) {
    if (pipelineType === "rag" && hasLinkSources) {
      return mixedPreferredGuideLabel(uploadLabel, linkHosts);
    }
    if (pipelineType === "fallback_web" || (hasLinkSources && pipelineType !== "rag")) {
