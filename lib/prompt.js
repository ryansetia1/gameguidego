...
export const VISUAL_SEARCH_QUERY_INSTRUCTION = `You write ONE short Google image search query in English for what a game item, character, boss, enemy, or location looks like.

Output ONLY the query text. No quotes, markdown, or explanation.
- Include the game name and platform when they are known
- Name the specific subject clearly
- Do NOT add: icon, sprite, image, picture, wallpaper, render, artwork, fanart, screenshot, png, jpg
- Keep it under 12 words
- Example: Funky Kong Mario Kart Wii`;

/**
 * @param {{ game?: string; platform?: string; question: string; searchTopic?: string; subject?: string }} params
 */
export function buildVisualSearchQueryPrompt({
  game,
  platform,
  question,
  searchTopic = "",
  ...
  const context = String(searchTopic || subject || "")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 400);
  return `Game: ${game || "unspecified"}
...
Player question:
${question}
${context ? `\nRetrieval hint (subject only, do not copy filler):\n${context}` : ""}`;
}
