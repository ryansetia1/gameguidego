...
export function buildPrompt({
  ...
  const imageBlock =
    imageCount > 0
      ? `The player attached ${imageCount} image(s) with this question (e.g. a screenshot or photo of where they are stuck). Use them as visual context — identify the exact screen, location, item, enemy, or menu shown, and read any dialog or text present. ${IMAGE_CHARACTER_RULES} Prioritise what they depict over guesses.\n\n`
      : "";
...
If image(s) are attached, they are the visual context for the question ("this boss", "here", "this item"): identify what they show (boss, enemy, location, screen, item, menu, in-game dialog, or character) and put that concrete subject into the query. ${IMAGE_REWRITE_CHARACTER_RULES}
...
If image(s) are attached, they are the visual context for the question ("this boss", "here", "this item"): identify what they show (boss, enemy, location, screen, item, menu, in-game dialog, or character) and describe that concrete subject in the query. ${IMAGE_REWRITE_CHARACTER_RULES}
...
export function buildRewritePrompt({ question, history = [], imageCount = 0, game = "", platform = "" }) {
  ...
  const imageBlock =
    imageCount > 0
      ? `The player attached ${imageCount} image(s) as visual context for this question. Identify the specific boss, enemy, location, screen, item, menu, in-game dialog, or character shown and fold it into the query. ${IMAGE_REWRITE_CHARACTER_RULES}\n\n`
      : "";
