...
If image(s) are attached, they are the visual context for the question ("this boss", "here", "this item"): identify what they show (the specific boss, enemy, location, screen, item, or menu) and put that concrete subject into the query.
...
If image(s) are attached, they are the visual context for the question ("this boss", "here", "this item"): identify what they show (the specific boss, enemy, location, screen, item, or menu) and describe that concrete subject in the query.
Write 1-2 clear sentences (up to about 60 words). Include enough context that someone reading only this query would know exactly which part of the walkthrough to look up.
Do NOT include the game title or platform — those are added separately.
 ...
 * @param {number} [params.imageCount]
 */
export function buildRewritePrompt({ question, history = [], imageCount = 0 }) {
  const conversation = history
    .map((turn) => `${turn.role === "user" ? "Player" : "Guide"}: ${turn.content}`)
    .join("\n");
  ...
  const imageBlock =
    imageCount > 0
      ? `The player attached ${imageCount} image(s) as visual context for this question. Identify the specific boss, enemy, location, screen, item, or menu shown and fold it into the query.\n\n`
      : "";

  return `${conversationBlock}${imageBlock}Latest question:\n${question}`;
}
