...
${playerBlock}${memoryBlock}${conversationBlock}${spoilerBlock}${preferredDirective}${webSupplementDirective}Web research (supporting evidence, may be incomplete or irrelevant):
${research}
...
const TOPIC_TITLE_FIRST_TURN_APPEND = `
First-turn thread label: extend the JSON shape to {"answer":"...","highlights":[...],"spoilers":[...],"topicTitle":"..."}.
- "topicTitle": very short label for this saved thread (first message only)
- Max 8 words, under 60 characters total
- Name the subject (boss, quest, item, build, mechanic) — not the whole question
...
export function summarizeSystemInstruction(isFirstTurn = false) {
  if (!isFirstTurn) return SYSTEM_INSTRUCTION;
  return `${SYSTEM_INSTRUCTION}${TOPIC_TITLE_FIRST_TURN_APPEND}`;
}
