...
export const TOPIC_TITLE_INSTRUCTION = `You label a saved game chat thread with a very short topic title.
Respond with ONLY a JSON object shaped like {"title":"..."}.
...
/**
 * @param {{ game?: string; platform?: string; question: string; answer: string }} params
 */
export function buildTopicTitlePrompt({ game, platform, question, answer }) {
  const answerExcerpt = String(answer || "")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 800);
  return `Game: ${game || "unspecified"}
