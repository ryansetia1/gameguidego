 ...
 * @typedef {{ title: string, content: string, preferred?: boolean, url?: string }} Source
 */

import { buildSpoilerBlock, buildSpoilerOutputRules } from "./spoiler-prefs.js";
import { buildPlayerMemoryPromptBlock } from "./player-memory.js";
import { isProgressFollowUp } from "./guide-progress.js";
import { JOURNAL_SOURCE_URL } from "./player-journey.js";
...
const SUMMARIZE_JSON_FIELDS = `- "answer": your full guide text as one string, in the player's language. Inside it, start with a short direct answer, then give clear numbered steps when the task is procedural; mention prerequisites, missable items, and version or platform differences when relevant; keep it concise.
...
const SUMMARIZE_JSON_SCHEMA = `Respond with ONLY a JSON object — no markdown fences, no text before or after it — shaped exactly like:
{"answer":"...","highlights":[...],"spoilers":[...]}
${SUMMARIZE_JSON_FIELDS}`;

const SUMMARIZE_JOURNAL_FIELDS = `- "journalReminder": when the player states NEW concrete progress (party, location, items, quest step, build) not already covered in PLAYER JOURNAL excerpts this turn, a short internal note (max 120 chars). Empty string otherwise.
- "journalReminderSummary": one-line toast for the player when journalReminder is set (max 80 chars, same language as the question). Empty when journalReminder is empty.
- Never invent progress in journalReminder fields. Empty both for question-only turns or when facts are already in PLAYER JOURNAL excerpts.`;

const SUMMARIZE_JSON_SCHEMA_JOURNAL = `Respond with ONLY a JSON object — no markdown fences, no text before or after it — shaped exactly like:
{"answer":"...","highlights":[...],"spoilers":[...],"journalReminder":"...","journalReminderSummary":"..."}
${SUMMARIZE_JSON_FIELDS}
${SUMMARIZE_JOURNAL_FIELDS}`;

const SUMMARIZE_JSON_SCHEMA_FIRST_TURN_JOURNAL = `Respond with ONLY a JSON object — no markdown fences, no text before or after it — shaped exactly like:
{"answer":"...","highlights":[...],"spoilers":[...],"topicTitle":"...","journalReminder":"...","journalReminderSummary":"..."}
You MUST include every key above. "topicTitle" is REQUIRED on this first message — never omit it.
${SUMMARIZE_JSON_FIELDS}
- "topicTitle": REQUIRED saved-thread label for this first message only.
- Max 8 words, under 60 characters total.
- Name the subject (boss, quest, item, area, build, mechanic) — NEVER paste or paraphrase the player's full question.
- Use the same language as the player's question.
- Do not include the game name or platform.
- Do not put major spoilers in the title.
- Good: "Tujuan kastil terbalik", "Malenia phase 2", "Estus shard locations"
- Bad: "aku kayanya udah 5% progress semenjak castle terbalik", "How do I beat this boss"

FIRST MESSAGE focus: the player's question sets the thread topic. Open "answer" with a direct reply to what they asked — their goal, confusion, or next step — before extra context. "topicTitle" must name that same focus in a few words.
${SUMMARIZE_JOURNAL_FIELDS}`;

const SUMMARIZE_JSON_SCHEMA_FIRST_TURN = `Respond with ONLY a JSON object — no markdown fences, no text before or after it — shaped exactly like:
{"answer":"...","highlights":[...],"spoilers":[...],"topicTitle":"..."}
You MUST include every key above. "topicTitle" is REQUIRED on this first message — never omit it.
${SUMMARIZE_JSON_FIELDS}
- "topicTitle": REQUIRED saved-thread label for this first message only.
- Max 8 words, under 60 characters total.
 ...
 * @param {import("./player-memory.js").PlayerMemoryPromptInput | null} [params.playerMemory]
 * @param {boolean} [params.webSupplement] Player also asked for web search with a guide hit
 * @param {boolean} [params.isFirstTurn] First message in a new saved thread
 */
export function buildPrompt({
  ...
  const progressFollowUpDirective =
    hasPreferred && isFollowUp && isProgressFollowUp(question)
      ? "PROGRESS FOLLOW-UP (strict): The player is continuing from where they left off in this chat. " +
        "Build your answer primarily from the FIRST [PREFERRED GUIDE] excerpt below. It was ranked as " +
        "the best match for their current position. Only use a later excerpt if the first one clearly " +
        "does not apply to where they said they are. Do not send them to obtain an item they already " +
        "got earlier in this conversation. Do not jump to a different dungeon or area arc.\n\n"
      : "";
  const preferredDirective = hasPreferred
    ? "The player chose specific guide(s) (marked PREFERRED GUIDE). For this game, " +
      ...
      "their exact names. Multiple excerpts may describe different points in the walkthrough; " +
      "use only excerpts consistent with the player's stated location and progress. If excerpts " +
      "conflict, prefer steps that immediately follow the event they described, not a later " +
      "upgrade or after leaving the area. Do not combine steps from excerpts that are not " +
      "sequential in the same arc. When two excerpts name similar rewards with different steps, " +
      "prefer the excerpt without upgraded or tiered labeling unless they asked for the upgrade. " +
      "If the specific answer genuinely isn't in them, say so in one line, then fall back to your " +
      "own knowledge and the other sources.\n\n"
    : "";

  const webSupplementDirective =
    webSupplement && hasPreferred
      ? "The player also asked to check the web this turn. Keep the preferred guide primary for " +
        "steps and item names. Use web snippets for patches, meta, and community tips. If they " +
        "conflict, note it briefly and default to the guide unless the question is about " +
        "changing or meta info.\n\n"
      : "";
  ...
  const research = sources.length
    ? sources
        .map((source, index) => {
          const label =
            source.url === JOURNAL_SOURCE_URL
              ? "[PLAYER JOURNAL | the player's saved progress for this game]"
              : source.preferred
  ...
  // Name only on the first turn; follow-ups get an explicit no-greeting rule so the
  // model stops opening every reply with "Halo <name>!".
  const playerBlock = !trimmedName
    ? ""
    : isFollowUp
      ? `This is a follow-up in an ongoing chat. Do NOT open with a greeting or the player's name (no "Halo ${trimmedName}!", "Hi again", etc.) — just answer.\n\n`
  ...
  const memoryBlock = buildPlayerMemoryPromptBlock(playerMemory);

  const firstTurnBlock = isFirstTurn
    ? "This is the first message in a new saved thread. Your JSON MUST include \"topicTitle\" — a short label naming the subject (not the full question). Open \"answer\" with a direct reply to what they asked.\n\n"
    : "";

  return `Game: ${game || "unspecified"}
Platform: ${platform || "unspecified"}

${playerBlock}${memoryBlock}${conversationBlock}${spoilerBlock}${progressFollowUpDirective}${preferredDirective}${webSupplementDirective}${firstTurnBlock}Web research (supporting evidence, may be incomplete or irrelevant):
${research}
...
Output the query text on the FIRST line: no quotes, no labels, no explanation, under 15 words. Expand vague phrasing into concrete, searchable terms. If the question is already standalone, just translate/normalise it to English.

If (and only if) the player is asking what something LOOKS LIKE — its appearance, sprite, portrait, icon, or on-screen look, in any language — add a SECOND line exactly: "VISUAL: <subject>". The subject is the concrete thing (the specific boss, enemy, character, item, or location), resolved from the conversation history when the question uses a reference ("it", "that", "rupanya", "nya"). Name the subject only — never the game title, and never the question word itself. If the question is not about appearance, output only the first line and no VISUAL line.`;
...
Do NOT include the game title or platform — those are added separately.
Output the retrieval query first. If (and only if) the player is asking what something LOOKS LIKE — its appearance, sprite, portrait, icon, or on-screen look, in any language — add a final line exactly: "VISUAL: <subject>", naming the concrete subject (the specific boss, enemy, character, item, or location), resolved from the conversation history for references ("it", "that", "rupanya", "nya"). Name the subject only — never the game title, never the question word. If the question is not about appearance, add no VISUAL line.
If the question is already standalone, expand it with the relevant context from the conversation anyway.`;

/** @param {boolean} [isFirstTurn] @param {boolean} [journeyEnabled] */
export function summarizeSystemInstruction(isFirstTurn = false, journeyEnabled = false) {
  let jsonSchema = SUMMARIZE_JSON_SCHEMA;
  if (journeyEnabled) {
    jsonSchema = isFirstTurn ? SUMMARIZE_JSON_SCHEMA_FIRST_TURN_JOURNAL : SUMMARIZE_JSON_SCHEMA_JOURNAL;
  } else if (isFirstTurn) {
    jsonSchema = SUMMARIZE_JSON_SCHEMA_FIRST_TURN;
  }
  const core = SYSTEM_INSTRUCTION.replace(SUMMARIZE_JSON_SCHEMA, jsonSchema);
  return core;
}
