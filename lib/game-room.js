...
/** True when Supabase rejects chat-topics migration columns. */
export function isTopicColumnDbError(error) {
  const msg = error?.message ?? "";
  return (
    /column .*(title|spoiler_major).* does not exist/i.test(msg) ||
    /Could not find the '(title|spoiler_major)' column/i.test(msg)
  );
}
...
/** Strip topic-only columns for legacy `chats` rows (pre chat-topics.sql). */
export function chatPayloadWithoutTopicColumns(payload) {
  const { title, spoiler_major, ...rest } = payload;
  return rest;
}