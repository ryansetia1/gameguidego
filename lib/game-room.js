...
import { guideUrlsFromChat, guideUrlsPayload, normalizeGuideUrlList } from "./guide-urls.js";

/**
 * Canonical game identity key. Shared by chat-room grouping and per-game memory so
 * "Assassin's Creed: Brotherhood" and "Assassins Creed Brotherhood" resolve to one
 * key. Quotes/apostrophes are deleted (so `Assassin's` → `assassins`); other
 * punctuation becomes a space (so `Spider-Man` matches `Spider Man`). Idempotent, so
 * re-running it on an already-normalized key is a no-op (the backfill relies on this).
 * @param {string} game
 */
...
/**
 * Pre-strengthen normalization (lowercase + collapse only). Kept so memory rows saved
 * under the old key can still be found before the backfill re-keys them.
 * ponytail: transition-only; delete once scripts/backfill-game-keys.mjs has run.
 * @param {string} game
 */
export function legacyNormGameKey(game) {
  return String(game ?? "").replace(/\s+/g, " ").trim().toLowerCase();
}
...
/**
 * Merge two chat rows: topic fields follow updated_at; shared room meta unions guides
 * and keeps the richest cover/year so a guide added in UI isn't lost when local wins
 * on equal timestamps.
 * @param {import("./supabase").Chat} a
 * @param {import("./supabase").Chat} b
 * @returns {import("./supabase").Chat}
 */
export function mergeChatRows(a, b) {
  const newer = topicActivityMs(a) >= topicActivityMs(b) ? a : b;
  const older = newer === a ? b : a;
  const merged = { ...older, ...newer };
  const olderUrls = guideUrlsFromChat(older);
  const newerUrls = guideUrlsFromChat(newer);
  // Shorter newer list = authoritative removal when non-empty; empty newer keeps remote guides.
  const urls =
    newerUrls.length < olderUrls.length && newerUrls.length > 0
      ? newerUrls
      : normalizeGuideUrlList([...olderUrls, ...newerUrls]);
  Object.assign(merged, guideUrlsPayload(urls));
  if (!String(merged.cover_url ?? "").trim() && String(older.cover_url ?? "").trim()) {
    merged.cover_url = older.cover_url;
  }
  if (!String(merged.release_year ?? "").trim() && String(older.release_year ?? "").trim()) {
    merged.release_year = older.release_year;
  }
  return merged;
}
...
export function mergeChatsFromServer(local, remote) {
  ...
  for (const row of local) {
    ...
    map.set(row.id, mergeChatRows(row, existing));
  }
