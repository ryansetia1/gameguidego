...
const FOCAL_ITEM_PATTERNS = [
  /\b(?:obtaining|acquiring|obtained|acquired)\s+(?:the\s+)?([a-z0-9][\w\s'-]{2,48}?)(?:\s+from|\s+in|\s+inside|\s*,|\s+following|\s+what|\s+please|$)/i,
 ...
 * @returns {string}
 */
function extractQueryFocalItem(queryText) {
...
export function rescoreGuideChunks(input) {
  ...
  const queryPostAcquisition =
    ...
    (/\b(?:next steps|what to do immediately|kemana lagi|following the steps)\b/i.test(queryText) ||
  ...
  const scored = chunks.map((chunk) => {
    ...
    if (queryPostAcquisition && hasPostAcquireContinuation(body)) {
      delta += 0.12;
      reasons.push("continuation_boost");
    }

    if (queryEarly && !queryPostAcquisition && hasNearbyAcquisition(body, queryFocalTokens)) {
