...
const FOCAL_ITEM_PATTERNS = [
  /\b(?:getting|got|receive|received|obtain|obtained|acquired|picked up|open(?:ed)?(?:\s+the)?\s+chest(?:\s+(?:for|to get))?)\s+(?:the\s+)?([a-z0-9][\w\s'-]{2,48}?)(?:\s+from|\s+in|\s+inside|\s*,|\s+after|\s+where|\s+and|\s+setelah|$)/i,
...
export function rescoreGuideChunks(input) {
  ...
  const scored = chunks.map((chunk) => {
    ...
    if (queryEarly && hasNearbyAcquisition(body, queryFocalTokens)) {
