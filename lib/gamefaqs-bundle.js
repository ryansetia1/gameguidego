...
/** Min extracted chars to treat a FAQ root as a complete single-page guide. */
export const SINGLE_PAGE_MIN_CHARS = 400;
...
/**
 * GameFAQs print-view URL for ingest extract (`?print=1`). Often returns the
 * full guide text instead of a short interactive-page slice.
 * @param {string} rawUrl
 * @returns {string | null}
 */
...
/**
 * Convert a GameFAQs section title to its URL slug.
 * "Part 1: The First Steps" -> "part-1-the-first-steps"; "Legalities & Credit" ->
 * "legalities-and-credit".
 * @param {string} title
 * @returns {string}
 */
export function slugFromGamefaqsTitle(title) {
  return String(title || "")
    .toLowerCase()
    .replace(/&/g, " and ")
    .replace(/['’]/g, "")
...
/**
 * Fallback TOC discovery when Tavily flattens the sidebar links to plain text (no
 * hrefs), so parseGamefaqsTocFromHtml finds nothing. GameFAQs walkthrough sections are
 * reliably "Part N: Title" — extract those and rebuild the URLs from the slug. These
 * are CANDIDATES: ingest attempts them and the failed-page tracking drops any that
 * don't resolve, so a mis-parsed title can't silently break anything.
 * @param {string} content
 * @param {{ faqId: string; canonicalUrl: string }} bundle
 * @returns {{ title: string; url: string; slug: string }[]}
 */
export function parseGamefaqsTocByTitles(content, bundle) {
  if (!content || typeof content !== "string" || !bundle?.canonicalUrl) return [];
  const pages = [];
  const seen = new Set();
  // Bound each title at the next "Part N:" marker or a short cap so flattened body
  // text can't bleed into the slug.
  const re = /part\s+(\d+)\s*[:.\-]\s*([a-z0-9][a-z0-9 ',&-]{1,50}?)(?=\s+part\s+\d+\s*[:.\-]|\s{2,}|$)/gi;
  let match;
  while ((match = re.exec(content)) !== null) {
    const num = match[1];
    const rawTitle = match[2].trim().replace(/\s+/g, " ");
    const title = `Part ${num}: ${rawTitle}`;
    const slug = slugFromGamefaqsTitle(title);
    if (!slug || seen.has(slug) || shouldSkipGamefaqsSlug(slug)) continue;
    seen.add(slug);
    pages.push({ slug, title, url: `${bundle.canonicalUrl}/${slug}` });
    if (pages.length >= MAX_BUNDLE_PAGES) break;
  }
  return pages;
}
...
/**
 * True when extracted FAQ HTML is a full single-page guide (long text, no sibling
 * section TOC). Multi-page bundles return false once the TOC lists 2+ sections.
 * @param {string} html
 * @param {{ faqId: string; canonicalUrl: string }} parsed
 * @param {number} [minChars]
 * @returns {boolean}
 */
export function isLikelySinglePageGamefaqsGuide(html, parsed, minChars = SINGLE_PAGE_MIN_CHARS) {
  if (!html || typeof html !== "string" || !parsed?.faqId) return false;
  const toc = parseGamefaqsTocFromHtml(html, parsed);
  if (toc.length > 1) return false;
  return html.length >= minChars;
}
 ...
 * @returns {string[]}
 */
export function buildGamefaqsPartDiscoveryQueries(parsed, maxPart = 12) {
  const path = new URL(parsed.canonicalUrl).pathname;
  const terms = gamefaqsDiscoveryTerms(parsed);
  const queries = [];
  // One query per part number. The TOC-extract path already covers whole-section
  // slugs and `/walkthrough`, so this sparse-discovery fallback stays bounded
  // (was ~50 advanced Tavily calls: 25 part + 15 walkthrough-part + 10 slugs).
...
/**
 * Print-view root ingest stores all chunks on the FAQ root URL — map them to discovery slugs for the panel.
 * @param {{ slug: string; title: string; url: string }[]} discoveryPages
 * @param {Map<string, number>} indexedByUrl
 * @param {string} canonicalUrl
 * @param {string} faqId
 * @returns {{ slug: string; title: string; url: string; indexed: boolean; chunks: number }[] | null}
 */
export function expandRootPrintBundleIndexPages(discoveryPages, indexedByUrl, canonicalUrl, faqId) {
  if (!discoveryPages?.length) return null;
  let rootUrl;
  try {
  ...
  const rootChunks = indexedByUrl.get(rootUrl) ?? 0;
  if (!rootChunks) return null;

  const hasSluggedChunks = [...indexedByUrl.entries()].some(([guideUrl, count]) => {
    if (!count) return false;
    try {
      const normalized = new URL(guideUrl);
      normalized.hostname = normalized.hostname.toLowerCase();
      if (normalized.pathname.length > 1) normalized.pathname = normalized.pathname.replace(/\/+$/, "");
      if (normalized.toString() === rootUrl) return false;
    } catch {
      return false;
    }
    return Boolean(slugFromGamefaqsPageUrl(guideUrl, faqId));
  });
  if (hasSluggedChunks) return null;

  const perPage = Math.max(1, Math.floor(rootChunks / discoveryPages.length));
