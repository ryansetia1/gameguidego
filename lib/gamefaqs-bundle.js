...
/**
 * Parse GameFAQs TOC entries whose markdown links are RELATIVE (`[Part 1: The First
 * Steps](part-1-the-first-steps)`) rather than absolute (`/faqs/{id}/…`). Tavily emits
 * this shape for some guides (e.g. faqs/80674), so the absolute-path regex in
 * parseGamefaqsTocFromHtml finds nothing. The relative href IS the section slug, so we
 * rebuild the absolute URL from it. Absolute links (`[Home](/)`, `http…`) and file
 * paths are excluded by the slug pattern; noise slugs are dropped by shouldSkip.
 * @param {string} content
 * @param {{ faqId: string; canonicalUrl: string }} bundle
 * @returns {{ title: string; url: string; slug: string }[]}
 */
export function parseGamefaqsTocLinks(content, bundle) {
  if (!content || typeof content !== "string" || !bundle?.canonicalUrl) return [];
  const pages = [];
  const seen = new Set();
  // [any title](relative-slug) — slug must start alphanumeric (excludes /paths, #anchors, http).
  const linkRe = /\[[^\]]+\]\(([a-z0-9][a-z0-9-]{1,80})\)/gi;
  let match;
  while ((match = linkRe.exec(content)) !== null) {
    const slug = match[1].toLowerCase();
    if (shouldSkipGamefaqsSlug(slug) || seen.has(slug)) continue;
    seen.add(slug);
    pages.push({ slug, title: titleFromGamefaqsSlug(slug), url: `${bundle.canonicalUrl}/${slug}` });
    if (pages.length >= MAX_BUNDLE_PAGES) break;
  }
  pages.sort((a, b) => gamefaqsPageOrder(a.slug) - gamefaqsPageOrder(b.slug) || a.title.localeCompare(b.title));
  return pages;
