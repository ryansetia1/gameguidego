...
/**
 * @param {string} text
 * @returns {{ answer: string, highlights: Highlight[], spoilers: SpoilerReveal[], spoilerRisk: boolean, topicTitle: string }}
 */
export function parseSummary(text) {
  const raw = text.trim();
  if (!raw) {
    return { answer: "", highlights: [], spoilers: [], spoilerRisk: false, topicTitle: "" };
  }
  ...
  if (start === -1 || end <= start) {
    return { answer: raw, highlights: [], spoilers: [], spoilerRisk: true, topicTitle: "" };
  }
  ...
  try {
    ...
    if (!answer) {
      return { answer: "", highlights: [], spoilers: [], spoilerRisk: true, topicTitle: "" };
    }
    const highlights =
    ...
    const topicTitle =
      "topicTitle" in parsed && typeof parsed.topicTitle === "string"
        ...
        : "";
    return { answer, highlights, spoilers, spoilerRisk, topicTitle };
  } catch {
    return { answer: raw, highlights: [], spoilers: [], spoilerRisk: true, topicTitle: "" };
  }
}
