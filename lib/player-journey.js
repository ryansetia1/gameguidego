...
export const JOURNEY_DISABLE_CONFIRM = "Turn off progress tracking?";
export const JOURNEY_CLEAR_ALL_CONFIRM =
  "Clear all progress journals? This cannot be undone.";
export const JOURNEY_PAUSED_HINT =
  "Progress tracking is off. Your saved journals stay here until you turn it back on or clear them.";
export const JOURNEY_EMPTY_HINT =
  "Nothing saved yet. Share where you are in chat, or tap Add to jot it down.";
export const JOURNEY_ADD_LABEL = "Add";
export const JOURNEY_EDIT_LABEL = "Edit";
export const JOURNEY_UPDATE_LABEL = "Update journal";
/** ~5 lines at game-card width; beyond this, preview + expand in UI. */
export const JOURNEY_LONG_BODY_CHARS = 280;
export const JOURNEY_SUMMARY_PREVIEW_CHARS = 72;

/** @param {string} text @param {number} [max] */
export function journalBodyPreview(text, max = JOURNEY_SUMMARY_PREVIEW_CHARS) {
...
/** @param {string} text */
export function isLongJournalBody(text) {
  return String(text ?? "").trim().length > JOURNEY_LONG_BODY_CHARS;
}
...
/** User-facing copy when a manual journal update is skipped. */
export function journalUpdateSkipMessage(skip, context = {}) {
  if (skip === "empty_delta") {
    return context.hadPriorSync
      ? "No new chat messages since the last update."
      : "Share progress in chat first, then tap Update journal.";
  }
  if (skip === "in_flight") return "Journal update already in progress.";
  return "";
}
 ...
 * @param {string} userId
 */
export async function disablePlayerJourney(supabase, _userId) {
