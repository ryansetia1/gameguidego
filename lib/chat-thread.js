import { coerceMessageVariant, coerceMessages, coerceIllustration, WRITING_ANSWER_PLACEHOLDER } from "./chat-messages.js";
...
export function responseRowToVariant(row) {
  ...
  const illustration = coerceIllustration(row.illustration);
  return {
...
export function buildMessagesFromNormalized(turns, responses, states) {
  ...
  for (const turn of sortedTurns) {
    ...
    messages.push({
      ...
      ...(active.illustration ? { illustration: active.illustration } : {}),
...
/**
 * Tail sync for follow-ups; full sync when edit/retry drops the thread tail.
 *
 * @param {Array<Record<string, unknown>>} priorMessages
 * @param {Array<Record<string, unknown>>} visibleMessages
 */
export function threadSyncModeForTurn(priorMessages, visibleMessages) {
  return priorMessages.length < visibleMessages.length ? "full" : "tail";
}

/**
 * True when the thread tail is ready for server/client assistant merge.
 *
 * @param {Array<Record<string, unknown>> | null | undefined} messages
 */
export function threadReadyForAssistantMerge(messages) {
  ...
  return (
    last.role === "assistant" &&
    last.content === WRITING_ANSWER_PLACEHOLDER
  );
}
...
/**
 * Copy assistant-only fields missing from the primary thread (e.g. illustration
 * dropped by older normalized rows). ponytail: field list, not a generic merge.
 *
 * @param {Array<Record<string, unknown>>} primary
 * @param {Array<Record<string, unknown>> | null | undefined} fallback
 * @returns {Array<Record<string, unknown>>}
 */
export function mergeAssistantFieldsFromLegacy(primary, fallback) {
  if (!Array.isArray(primary) || !Array.isArray(fallback) || !fallback.length) {
    return primary;
  }
  ...
  return primary.map((message) => {
    ...
    while (
      fallbackAssistantIdx < fallback.length &&
      fallback[fallbackAssistantIdx]?.role !== "assistant"
    ) {
      fallbackAssistantIdx += 1;
    }
    const legacy = fallback[fallbackAssistantIdx];
    fallbackAssistantIdx += 1;
    if (!legacy) return message;

    let next = message;
    if (!message.illustration && legacy.illustration) {
      next = { ...next, illustration: legacy.illustration };
    }

    const variants = Array.isArray(message.variants) ? message.variants : null;
    const legacyVariants = Array.isArray(legacy.variants) ? legacy.variants : null;
    if (variants?.length && legacyVariants?.length) {
      const mergedVariants = variants.map((variant, index) => {
        const legacyVariant = legacyVariants[index];
        if (
          legacyVariant &&
          typeof legacyVariant === "object" &&
          !variant.illustration &&
          legacyVariant.illustration
        ) {
          return { ...variant, illustration: legacyVariant.illustration };
        }
        return variant;
      });
      if (mergedVariants.some((variant, index) => variant !== variants[index])) {
        next = { ...next, variants: mergedVariants };
      }
    }
      ...
      : [
          {
            ...
            illustration: lastAssistant.illustration,
          },
        ];
