...
/**
 * Release year via the public Steam Store appdetails endpoint (GetOwnedGames
 * does not include release date). Best-effort; returns "" on any failure.
 * @param {number} appId
 * @returns {Promise<string>}
 */
export async function fetchSteamReleaseYear(appId) {
  if (!Number.isFinite(appId) || appId <= 0) return "";
  const url = `https://store.steampowered.com/api/appdetails?appids=${Math.trunc(appId)}&filters=basic`;
  try {
    const response = await fetch(url, { signal: AbortSignal.timeout(10_000) });
    if (!response.ok) return "";
    const payload = await response.json();
    const block = payload?.[String(Math.trunc(appId))];
    if (!block?.success || !block.data) return "";
    return yearFromSteamReleaseDate(block.data.release_date?.date);
  } catch {
    return "";
  }
}
