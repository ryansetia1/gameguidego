...
| [rag-tuning-roadmap.md](./rag-tuning-roadmap.md) | **Phase D shipped** | RAG tuning backlog, plus the hybrid vector + exact-name retrieval that shipped 2026-08-06 and its live calibration across four guides |
...
| [player-journey-tracker.md](./player-journey-tracker.md) | **Planned** | Per-game progress journal: long-form RAG, mandatory auto-update (`journalReminder`), daily cap, solve-persist ordering gotchas, toast + `journal_*` traces |
...
| [guide-source-selector.md](./guide-source-selector.md) | **Shipped** | Per-turn multi-select RAG source in composer (Auto vs subset of attached guides) |
